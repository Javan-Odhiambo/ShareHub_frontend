...
import { useInnovationsFetchOneQuery, useInnovationsCommentsCreateMutation, useInnovationsCommentsListQuery, useInnovationsCommentsDeleteMutation, useInnovationsCommentsUpdatePatchMutation, useInnovationsCommentsUpdatePutMutation, useInnovationsCommentsReadQuery } from "@/redux/features/innovations/innovationsApiSlice"
...
import type { TComment as TypeComment } from "@/lib/types"
...
const InnovationDetailPage = ({ params }: InnovationDetailPageProps) => {
    ...
    const { data: innovation, isLoading: isGettingInnovation, error: errorGettingInnovation } = useInnovationsFetchOneQuery(id);
    const [createComment, { isLoading: isCreatingComment }] = useInnovationsCommentsCreateMutation();
    const { data: comments, isLoading: isGettingComment, error: errorGettingComments } = useInnovationsCommentsListQuery(id);
    ...
    return (
        isGettingInnovation || isGettingComment ? (
            <div>Loading...</div>
            ...
            <main>
                <section className='flex flex-col'>
                    ...
                    <div className="flex items-center justify-between mt-5 px-4 ">
                        <div className='flex items-center gap-3'>
                            <Avatar className='h-12 w-12'>
                                ...
                                <AvatarFallback className="p-2">{get_fallback_name(innovation?.author.first_name, innovation?.author.last_name)}</AvatarFallback>
                            </Avatar>
                            <div>
                    ...
                    <div className="flex p-4 my-2 justify-between bg-accent">
                        ...
                        <span
                            className="flex"
                            onClick={innovation?.is_bookmarked ? handleUnbookmark : handleBookmark}
                        >
                ...
                <section>
                    {
                        innovation?.banner_image ? (
                            <Image alt="Innovation Image" width={500} height={600} src={innovation?.banner_image} />
                ...
                <section className="p-7 space-y-4">
                    {comments?.length > 0 && comments?.map((comment, index) => (
                        <div className="border p-3 rounded-lg shadow-md" key={index}>
                            ...
                            <p>
                                Lorem ipsum dolor, sit amet consectetur adipisicing elit. Reprehenderit velit dignissimos voluptatem pariatur.
                            </p>
                        </div>
                    ))}
                </section>
